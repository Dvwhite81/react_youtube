...
const ChannelDetail = () => {
  ...
  return (
    <div>
      <div className="w-full bg-cover bg-center" style={{ height: "10rem" }}>
        <div className="flex items-center w-full justify-center h-full  bg-gray-900 bg-opacity-50" style={{ height: "10rem" }}>
          <img src={channelDetail?.brandingSettings?.image?.bannerExternalUrl} style={{ height: "10rem", width: "full" }} />
        </div>
      </div>


      <div className="mt-[-20px] ml-10 flex items-center space-x-4 ">
        <img className="border-4 border-white w-20 h-20 md:w-40 md:h-40 rounded-full" src={channelDetail?.snippet?.thumbnails?.high?.url} alt="" />
        <div className=" mt-4 font-medium text-3xl dark:text-white">
          <div>{channelDetail?.snippet?.title}</div>
          <div className="w-3/5 text-sm text-gray-500 dark:text-gray-400">{channelDetail?.snippet?.description}</div>
